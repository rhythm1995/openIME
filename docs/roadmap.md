...
> 未实现功能的结构化需求清单。来源：[competitive-research.md](./competitive-research.md) 的竞品调研 + 产品判断。
> **已实现**的功能见 [README](../README.md) 路线图（✅ 段）与 [development.md](./development.md) 测试矩阵。
...
### R8. 风格包分享（市场 / 导入导出）
- **价值** 中 · **难度** ★★★★ · 来源 F2 / OpenLess
- **描述**：社区分享风格包。轻量版先做 JSON 文件导入导出；完整市场需后端服务 + GitHub OAuth + 审核（增长期）。
- **方案（轻量）**：风格包 CRUD 已有（F1 ✅），加「导出 JSON / 导入 JSON」。
- **验收**：导出一个风格包为 JSON，另一台机器导入即可用。
- **依赖**：F1（✅）。
...
### R10. 鼠标侧键 / 线控触发
- **价值** 低中 · **难度** ★★★（侧键）/ ★★（线控）· 来源 A3, A4
- **描述**：支持鼠标 X1/X2 侧键、耳机线控（MediaPlayPause）触发录音。
- **方案**：侧键用 CGEventTap 监听 `kCGEventOtherMouseDown`；线控注册媒体键全局快捷键。
- **验收**：可配置侧键 / 线控为录音键。
- **依赖**：无。
