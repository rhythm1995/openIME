...
选中一段文字 → 按 **划词问答快捷键** 弹出问答浮窗 → 按录音键语音提问，回答流式显示，支持多轮。浮窗内可复制、插入光标、刷新选区；关窗即清空上下文。需要云端 LLM 凭据。
...
设置分为两组侧边栏入口：**语音配置**（快捷键与触发、识别引擎、音频、文件转录、插入兼容性、应用行为、权限、功能测试）与 **AI 增强配置**（润色、角色 / 风格包、翻译、划词问答）。快捷键相关设置在 语音配置 → 快捷键：
...
- **划词问答快捷键**：可选，如 `Cmd+Shift+;`；选中文字后按下弹出问答浮窗。
