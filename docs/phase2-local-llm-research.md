...
| **云端兜底** | 本地不可用/失败/关闭时走云端 OpenAI 兼容 API | 自动或用户强制 |
...
| 云端 | 任意 OpenAI 兼容服务（`qwen-turbo` / `gpt-4o-mini` 等） | — | 0 本地 | 最强 | 商业 API | 兜底 / 强制云端 |
...
5. 同属「通义系」开源模型，本地/云端风格一致性更好（产品叙事也顺）。
...
- 云端：**OpenAI 兼容** `chat/completions`（与 ASR 凭据完全分离，独立填写 endpoint / api_key）。  
...
polish_cloud_model: ""              // 可选，留空时请求不带 model 字段
...
| **P2 云端 polish** | OpenAI 兼容 chat；独立凭据；超时与错误回退 |
...
4. **云端**：任意 OpenAI 兼容服务，`qwen-turbo` / `gpt-4o-mini` 级。  
...
- [阿里云通义模型列表](https://help.aliyun.com/zh/model-studio/models)  
...
| 云端 CloudPolishProvider（OpenAI 兼容） | ✅ |
